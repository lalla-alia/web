...
function nextOrderKey(qaArray, cat){
  const count=(qaArray||[]).filter(x=>(x.cat && x.cat.trim() ? x.cat.trim() : 'عام')===cat).length;
  const bucket=Math.floor(count/100);
  const seq=(count%100)+1;
  return `0:${bucket}:${seq}`;
}
...
document.getElementById('saveBtn').innerHTML = ICONS.save;
document.getElementById('inboxBtn').innerHTML = ICONS.inbox;

/* ---------------- زر الحفظ اليدوي وصندوق الأسئلة الواردة (للمؤسس فقط) ---------------- */
...
if(lockSendBtn){
  lockSendBtn.onclick=async ()=>{
    ...
    statusEl.textContent='جارِ الإرسال...';
    const { error } = await supaClient.auth.signInWithOtp({ email });
    if(error){ statusEl.textContent='تعذّر الإرسال: '+error.message; return; }
...
if(lockVerifyBtn){
  lockVerifyBtn.onclick=async ()=>{
    ...
    if(error){ statusEl.textContent='رمز غير صحيح أو منتهي الصلاحية: '+error.message; return; }
...
function renderInboxList(items){
  ...
  items.forEach(item=>{
    ...
    const catInput=document.createElement('input');
    catInput.placeholder='التصنيف'; catInput.value='أسئلة الزوار';
    form.appendChild(catInput);
    box.appendChild(form);
    ...
    const bar=buildToolbar(aBox, ()=>{}, {media:false});
    box.appendChild(bar); box.appendChild(aBox);
    ...
    publishBtn.onclick=async ()=>{
      ...
      const cat=catInput.value.trim()||'أسئلة الزوار';
      targetPage.qa.push({ q:item.question, a:cleanContentHtml(aBox), cat:cat, order:nextOrderKey(targetPage.qa, cat) });
      save();
